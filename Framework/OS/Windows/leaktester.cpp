...
static	void	doCleanupLogOnFirstRun()
{
	if (cleanupLogOnFirstRun)
	{
		...
		OS_Core::GetDesktopFolder(aFolder);
		strcat(aFolder,"\\memoryleaks.txt");
		remove(aFolder);
...
static	const char	*memorySizeString(unsigned int size)
{
	static	char	str[90];
...
static	sAllocUnit	*findAllocUnit(const void *reportedAddress)
{
	...
	unsigned int	hashIndex = ((unsigned int)PtrToUlong( reportedAddress) >> 4) & (hashSize - 1);
...
static	size_t	calculateActualSize(const size_t reportedSize)
{
	...
	return reportedSize + paddingSize * sizeof(unsigned int) * 2;
}
	...
	for (unsigned int i = 0; i < hashSize; i++)
	{
		...
		while(ptr)
		{
			fprintf(fp, "%06d 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X %-8s    %c       %c    %s\r\n",
				...
				(unsigned int) PtrToUlong(ptr->reportedAddress), ptr->reportedSize,
				(unsigned int) PtrToUlong(ptr->actualAddress), ptr->actualSize,
				m_calcUnused(ptr),
...
void	*m_allocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
	try
	{
		...
		if (alwaysLogAll) log("%05d %-40s %8s            : %s", currentAllocationCount, ownerString(sourceFile, sourceLine, sourceFunc), allocationTypes[allocationType], memorySizeString((unsigned long)reportedSize));
		...
		unsigned int	hashIndex = ((unsigned int) PtrToUlong(au->reportedAddress) >> 4) & (hashSize - 1);
...
void	*m_reallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int reallocationType, const size_t reportedSize, void *reportedAddress)
{
	try
	{
		...
		if (alwaysLogAll) log("%05d %-40s %8s(%010p): %s", currentAllocationCount, ownerString(sourceFile, sourceLine, sourceFunc), allocationTypes[reallocationType], reportedAddress, memorySizeString((unsigned long)reportedSize));
		...
		if (oldReportedAddress != au->reportedAddress)
		{
			...
			{
				unsigned int	hashIndex = ((unsigned int) PtrToUlong(oldReportedAddress) >> 4) & (hashSize - 1);
			...
			hashIndex = ((unsigned int) PtrToUlong(au->reportedAddress) >> 4) & (hashSize - 1);
			if (hashTable[hashIndex]) hashTable[hashIndex]->prev = au;
			au->next = hashTable[hashIndex];
			au->prev = NULL;
			hashTable[hashIndex] = au;
...
bool	m_validateAllocUnit(const sAllocUnit *allocUnit)
{
	...
	for (unsigned int i = 0; i < paddingSize; i++, pre++, post++)
	{
		...
		//
		// J.R.: I commented this out because it seems to trip up in the D3DX library.
		//
...
void	m_dumpAllocUnit(const sAllocUnit *allocUnit, const char *prefix)
{
	...
	log("%sSize (reported)   : 0x%08X (%s)", prefix, allocUnit->reportedSize, memorySizeString((unsigned long)allocUnit->reportedSize));
	log("%sSize (actual)     : 0x%08X (%s)", prefix, allocUnit->actualSize, memorySizeString((unsigned long)allocUnit->actualSize));
