...
bool CSoundFile::ReadMO3(FileReader &file, ModLoadingFlags loadFlags)
{
	...
	*/
	...
	for(SAMPLEINDEX smp = 1; smp <= m_nSamples; smp++)
	{
		...
		} else if(smpHeader.compressedSize > 0)
		{
			...
			} else if(compression == MO3Sample::smpCompressionMPEG)
			{
				...
				{
					AddToLog(LogWarning, mpt::format(U_("Sample %1: Unsupported MP3(1)"))(smp));
			...
			{
				AddToLog(LogWarning, mpt::format(U_("Sample %1: Unsupported MP3(2)"))(smp));
				unsupportedSamples = true;
			}
		}
	}
		...
		for(SAMPLEINDEX smp = 1; smp <= m_nSamples; smp++)
		{
			...
			if(ov_open_callbacks(&sampleData, &vf, nullptr, 0, callbacks) == 0)
			{
				...
				{  // we do not support chained vorbis samples
					...
					{
						AddToLog(LogWarning, mpt::format(U_("Sample %1: Unsupported OggVorbis(1)"))(smp));
						unsupportedSamples = true;
					}
				} else
				{
			...
			{
				AddToLog(LogWarning, mpt::format(U_("Sample %1: Unsupported OggVorbis(2)"))(smp));
				unsupportedSamples = true;
			}
			...
			{
				AddToLog(LogWarning, mpt::format(U_("Sample %1: Unsupported OggVorbis(3)"))(smp));
				unsupportedSamples = true;
			}

#else  // !VORBIS

			AddToLog(LogWarning, mpt::format(U_("Sample %1: Unsupported OggVorbis(4)"))(smp));
			unsupportedSamples = true;
	...
	{
		AddToLog(LogWarning, U_("Some compressed samples (MO3) could not be loaded because they use an unsupported codec."));
	}
