...
//
// Uncomment comments above when done...
//
...
namespace OS_Core
{
	...
	void			Log(char* format, ...);					// Writes to the app.log file
	void			LogEnable(bool theState=true);
	...
	void			GetAppStorageFolder(char *theResult);
	void			GetCloudFolder(char *theResult);
	void			GetHomeFolder(char*theResult);
	void			GetPackageFolder(char *theResult);
	void			GetCacheFolder(char *theResult);
	void			GetTempFolder(char *theResult);
	void			LocationAlias(char* theFolder, char* theAlias);	// Lets you temporary re-route a location:// somewhere (i.e. sandbox:// goes to temp://, etc)
	...
	void 			SecureLog(char* theFilename, char* theData);
	...
	//
	inline char*	GetOSName() {return (char*)"linux";}
	void			GetOSVersion(char *theResult);
	char*			GetOSQuarantine();	// This is used to tag our system for settings that need to be specific to the system.
	...
	//
	inline bool		AllowThreads() {return true;}
	...
	bool			HasCloud();
	char			GetCloudStatus();
	bool			PutCloud(char* theKey, void* thePtr, int theSize, bool waitForResults=false);
	bool			GetCloud(char* theKey, void** thePtr, int* theSize);
	void			DeleteCloud(char* theKey);
	void			EnumCloud(Array<char *>&theArray);
	bool			IsCloudEmpty();
